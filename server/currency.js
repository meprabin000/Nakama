...
async function convert(req, res, next){
    ...
    if (Object.keys(queryParam).length == 1) { //only allows for one query param
        if (queryParam.yen != null && !isNaN(queryParam.yen) && Number(queryParam.yen) > 0){ //converts yen to usd and checks if the value is a number
        ...
        else if (queryParam.us != null && !isNaN(queryParam.us) && Number(queryParam.us) > 0){  //converts usd to yen and checks if the value is a number
